  ...
  const handlePasswordChange= async (event: React.ChangeEvent<HTMLInputElement>) =>{
   ...
   return(
      ...
      <div className="p-2 w-1/3 border-2 border-gray-600 flex flex-col justify-center items-start">
         ...
         <button className="text-gray-900 bg-gradient-to-r w-3/4 from-lime-200 via-lime-400 to-lime-500 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-lime-300 dark:focus:ring-lime-800 font-medium rounded-md text-sm px-5 py-2.5 text-center me-2 mb-2" 
          onClick={handleSubmit}>{loading}</button>
         </div>
         <p className="text-gray-100 p-3">Already have an account? <span><Link className="text-blue-300 hover:underline"href={'/login'}>Login</Link></span></p>
         </div>
      </div>   
   )
}
