...
const getTopicById = async (subId: string) => {
  try {
    const res = await fetch(`/api/getEditSubject/${encodeURIComponent(subId)}`, {
      cache: "no-store",
      method: "GET",
      headers: {
    ...
    console.error("Error fetching the subject", error, subId);
    throw error;
  }
};
...
export default function Update() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const data = await getTopicById(subId);
  ...
  const { subName, grade, credit } = subjectData;

  return <Edit id={subId!} subName={subName} grade={grade} credit={credit} />;
}
